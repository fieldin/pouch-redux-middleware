...
module.exports = {
  createPouchMiddleware: createPouchMiddleware,
  destroy: destroy
};
...
function createPouchMiddleware(_paths) {
  ...
  function metaDBListen(dbPaths, dispatch, initialBatchDispatched) {
    dbPaths.db.allDocs({ include_docs: true }).then(function (rawAllDocs) {
  ...
  function dataDBListen(dbPaths, dispatch, initialBatchDispatched) {
    findData(dbPaths).then(function (rawAllDocs) {
      var allDocs = rawAllDocs.reduce(function (result, pathDocs) {
        var docs = pathDocs.rows.map(function (doc) {
          return doc.doc;
        });
  ...
  function findData(dbPaths) {
    return Promise.all(dbPaths.paths.map(function (path) {
      return allDocs(dbPaths.db, path.entity);
    }));
  }
  ...
  function onInitialDataReceived(dbPaths, dispatch, initialBatchDispatched, allDocs) {
    dbPaths.paths.forEach(function (path) {
  ...
  function initMetaDB(db, options) {
    metaDBListen(dbsPaths[db], options.dispatch, function (err) {
      onInitialBatchDispatched(db, err);
    });
  }

  function initDataDB(db, options) {
    dataDBListen(dbsPaths[db], options.dispatch, function (err) {
      onInitialBatchDispatched(db, err);
    });
  }
  ...
  return function (options) {
    Object.keys(dbsPaths).forEach(function (db) {
      // <-- runs on each db
      if (db === "fieldin_meta") {
        initMetaDB(db, options);
      } else {
        // <-- this is a company_x db
  ...
  dbPaths.forEach(function (path) {
    ...
    if (changeDoc._deleted) {
      ...
      if (oldDoc) {
        ...
        if (new Date(changeDoc.updated_at) >= new Date(oldDoc.updated_at)) {
          path.propagateUpdate(changeDoc, dispatch);
        } else {
          path.propagateUpdate(oldDoc, dispatch);
        }
      } else {
